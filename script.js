...
function initPortfolio() {
  ...
  galleryTabs.innerHTML = allCategories.map((cat, idx) => `
    <button class="tab-btn ${idx === 0 ? 'active' : ''}" data-filter="${cat}">${cat}</button>
  `).join('');
...
if (openGalleryBtn) {
  openGalleryBtn.addEventListener('click', () => {
    const mainArea = document.querySelector('.carousel-section');
    if (mainArea) {
      mainArea.scrollTo({ top: galleryTabs.offsetTop - 20, behavior: 'smooth' });
    }
  });
}
...
const mainContentContainer = document.querySelector('.carousel-section');
...
function toggleBackToTop() {
  ...
  const sectionScrollTop = mainContentContainer.scrollTop; 
  const pageScrollTop = window.scrollY || document.documentElement.scrollTop; 

  if (sectionScrollTop > 300 || pageScrollTop > 300) { 
